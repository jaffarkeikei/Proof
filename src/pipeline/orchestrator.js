...
export class PipelineOrchestrator extends EventEmitter {
  ...
  async executePipeline(input) {
    ...
    try {
      ...
      this.emit('complete', {
        runId,
        status: 'completed',
        videoUrl: videoResult.videoUrl || `/videos/${runId}.mp4`,
  ...
  async generateDemoVideo(review, companyName) {
    ...
    logger.info('Demo video generated', { company: companyName, author: review.author });

    return {
      videoPath: `/tmp/demo-video-${Date.now()}.mp4`,
      videoUrl: null,  // Would be set to actual URL in production
      duration: 45,
      status: 'completed'
    };
  }
